...
2026-09-07 生产日志暴露客户端将终端/SQL 输出误填到 `model` 的情况。再次核对三个参考网关：
jwadow/hj 的模型字段均是普通字符串，chaogei 转换器接收模型字符串；它们没有提供可复用的此类防护。
本项目增加 **256 字节、禁止空白/控制字符** 的资源与日志安全边界，不宣称这是 Kiro 的模型枚举约束。
自定义别名、Unicode、命名空间 `/`、ARN `:` 和 `[1m]` 等后缀继续接受。非法模型在选账号前返回 400，
诊断仅保留 `[invalid-model]`，不截取原输入前缀；错误文本有界且不保留控制字符。
