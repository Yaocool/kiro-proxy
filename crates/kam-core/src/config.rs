...
pub struct LogConfig {
    ...
    /// `json` 或 `pretty`。
    pub format: String,
    /// 日志基础路径；空字符串使用数据目录下的 `logs/kamd.log`。
...
fn default_socket_path_from(kam_home: Option<&str>, xdg_runtime: Option<&str>) -> String {
    if let Some(home) = kam_home {
        return format!("{home}/admin.sock");
    }
    if let Some(runtime) = xdg_runtime {
        return format!("{runtime}/kam/admin.sock");
    }
    "/run/kam/admin.sock".into()
}
...
[server.tls]
enabled = false
# cert_path = "/etc/kam/fullchain.pem"
# key_path = "/etc/kam/privkey.pem"
...
"#,
    ...
    fn socket_path_honours_development_environment() {
        assert_eq!(
            default_socket_path_from(Some("/tmp/kam"), None),
            "/tmp/kam/admin.sock"
        );
        assert_eq!(
            default_socket_path_from(None, Some("/run/user/1000")),
            "/run/user/1000/kam/admin.sock"
        );
        assert_eq!(default_socket_path_from(None, None), "/run/kam/admin.sock");
    }
}
