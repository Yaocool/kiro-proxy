...
use kam_core::config::{Config, DEFAULT_CONFIG_TOML};
...
pub async fn ensure_layout(paths: &Paths) -> Result<BootstrapReport> {
    ...
    // DEFAULT_CONFIG_TOML 面向生产默认使用 /run。开发与容器使用 KAM_HOME 时，
...
mod tests {
    ...
    async fn accounts_start_empty_with_restrictive_permissions() {
        ...
        let accounts: Vec<kam_core::account::Account> = serde_json::from_str(
