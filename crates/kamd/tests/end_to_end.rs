//! 端到端测试：拉起真实 kamd，通过 Unix socket 走完整协议。

use std::path::{Path, PathBuf};
use std::process::Stdio;
use std::time::{Duration, Instant};

use kam_ipc::protocol::{decode_line, encode_line, Request, Response};
...
async fn wait_for_http(port: u16) {
    ...
    loop {
        ...
        assert!(Instant::now() < deadline, "kamd HTTP plane did not start");
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
}
...
async fn dotenv_is_loaded_before_daemon_startup_configuration() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_kamd"))
        .current_dir(workspace.path())
        .env_remove("KAM_HOME")
        .env_remove("KAM_HTTP_PORT")
        .env_remove("KAM_DISABLE_HTTP")
        .env_remove("RUST_LOG")
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .kill_on_drop(true)
        .spawn()
        .expect("spawn kamd from .env");
...
async fn restart_reloads_persisted_accounts() {
    let home = tempfile::tempdir().expect("tempdir");
    let socket = home.path().join("admin.sock");
    let mut first = Command::new(env!("CARGO_BIN_EXE_kamd"))
        .env("KAM_HOME", home.path())
        .env("KAM_HTTP_PORT", "0")
        .env("KAM_DISABLE_HTTP", "1")
        .env("RUST_LOG", "warn")
    ...
    let mut second = Command::new(env!("CARGO_BIN_EXE_kamd"))
        .env("KAM_HOME", home.path())
        .env("KAM_HTTP_PORT", "0")
        .env("KAM_DISABLE_HTTP", "1")
        .env("RUST_LOG", "warn")
