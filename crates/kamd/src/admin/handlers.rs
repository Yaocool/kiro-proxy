...
async fn handle_diagnose_endpoints(params: serde_json::Value) -> Handled {
    ...
    let endpoints = vec![
        (
            "codewhisperer",
            kam_kiro::endpoint::CODEWHISPERER_URL.to_string(),
        ),
        ("amazonq", kam_kiro::endpoint::AMAZONQ_URL.to_string()),
        (
            "oidc",
        ...
        async move {
            ...
            match client.post(&url).json(&serde_json::json!({})).send().await {
                ...
                Err(error) => serde_json::json!({
                    ...
                    "latency_ms":started.elapsed().as_millis(),
                    "error":kam_translate::sanitize_error_message(&error.to_string())
                }),
            }
        }
    }))
...
async fn handle_subscriptions(state: &Arc<AppState>, params: serde_json::Value) -> Handled {
    ...
    let subscriptions = state
        ...
        .await
        .map_err(|error| {
            RpcError::internal(kam_translate::sanitize_error_message(&error.to_string()))
        })?;
...
async fn handle_status(state: &Arc<AppState>) -> Handled {
    ...
    let hint = if total == 0 {
        Some("无可用账号，请先添加：kam account import".to_string())
    } else if enabled == 0 {
...
async fn handle_account_show(state: &Arc<AppState>, params: serde_json::Value) -> Handled {
    ...
    let preferred_endpoint = state
        .kiro()
        .endpoint_cache()
        .preferred(&account.id, kam_kiro::EndpointPurpose::Generation)
    ...
    for id in ids {
        match state.refresh_account_token(&pool, &id, true).await {
            ...
            Err(error) => refreshed.push(serde_json::json!({
                "id":id,"ok":false,"error":kam_translate::sanitize_error_message(&error.to_string())
            })),
        }
    }
...
async fn probe_account_inner(
    ...
    let request = kam_translate::OpenAiRequest {
        model: model.clone(),
        messages: vec![kam_translate::OpenAiMessage {
            role: "user".into(),
    ...
    let payload = kam_translate::openai_to_kiro(
        &request,
        &kam_translate::TranslationOptions::new(model, "CLI"),
    );
        ...
        Ok(response) => {
            ...
            match state.kiro().collect_events(response).await {
                Ok(events) => serde_json::json!({
                    "ok":events.iter().any(|event| matches!(event, kam_kiro::KiroEvent::AssistantResponse { .. })),
                    "endpoint":endpoint.name
                }),
                Err(error) => {
                    serde_json::json!({"ok":false,"error":kam_translate::sanitize_error_message(&error.to_string())})
                }
            }
        }
        Err(error) => {
...
fn handle_webhook_test(state: &Arc<AppState>, params: serde_json::Value) -> Handled {
    ...
    let event = WebhookEvent::new(
        ...
        "This is a test notification from kam.",
    );
...
#[cfg(test)]
mod tests {
    use kam_core::account::{Account, AuthMethod, Credentials};
