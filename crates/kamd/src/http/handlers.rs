...
use kam_kiro::{KiroError, KiroResponse};
use kam_notify::{WebhookEvent, WebhookEventKind};
use kam_pool::{AccountLease, PoolError};
use kam_translate::model::{apply_adaptive_thinking, map_model, thinking_enabled_for_model};
use kam_translate::{
...
pub(super) fn set_payload_model(payload: &mut kam_translate::KiroPayload, model: &str) {
    payload
        .conversation_state
        .current_message
        .user_input_message
...
pub(super) fn find_model_fallback(model: &str, models: &[kam_kiro::ModelInfo]) -> Option<String> {
...
async fn collect_nonstream_rounds(
    ...
    mut payload: kam_translate::KiroPayload,
    ...
    loop {
        ...
        let uses = decoded
            .tools
            .values()
            .map(|tool| kam_translate::KiroToolUse {
        ...
        payload = kam_translate::auto_continue_payload(&payload, &round_text, uses);
        tracing::info!(
            trace_id,
            account_id = %lease.account_id(),
            endpoint,
...
fn merge_round_usage(total: &mut kam_kiro::UsageInfo, addition: &kam_kiro::UsageInfo) {
...
pub(crate) fn fallback_models(config: &kam_core::config::Config) -> Vec<kam_kiro::ModelInfo> {
    let mut models = kam_kiro::static_models()
        .into_iter()
...
fn configured_model_info(model_id: String) -> kam_kiro::ModelInfo {
    kam_kiro::ModelInfo {
...
fn estimated_credits(
    input_tokens: u64,
    max_tokens: u32,
    config: &kam_core::config::PoolConfig,
) -> f64 {
...
mod model_tests {
    ...
    fn fallback_chooses_the_highest_lower_model_in_the_same_family() {
        let models = ["claude-opus-4-5", "claude-opus-4-6", "claude-sonnet-4-9"]
            .into_iter()
            .map(|model_id| kam_kiro::ModelInfo {
    ...
    fn credit_reservation_heuristic_is_configurable_and_capped() {
        let config = kam_core::config::PoolConfig {
            credit_estimate_per_1k_tokens: 2.0,
            credit_estimate_output_token_cap: 100,
            ..kam_core::config::PoolConfig::default()
        };
    ...
    fn fallback_models_use_catalog_and_keep_configured_targets() {
        let mut config = kam_core::config::Config::default();
