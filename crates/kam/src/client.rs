...
impl AdminClient {
    ...
    pub async fn call<T: DeserializeOwned>(
        ...
        let stream = UnixStream::connect(&self.socket).await.with_context(|| {
            format!(
                "无法连接 {}。kamd 未运行？可用 `systemctl status kamd` 检查服务",
                self.socket.display()
            )
        })?;
...
pub async fn resolve_socket(explicit: Option<String>) -> PathBuf {
    ...
    let paths = kam_core::paths::Paths::from_env();
    match kam_store::config_loader::load_config(&paths.config_file).await {
        Ok(config) => PathBuf::from(config.admin.socket),
        Err(_) => PathBuf::from(kam_core::config::Config::default().admin.socket),
    }
}
