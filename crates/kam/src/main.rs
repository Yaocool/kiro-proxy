...
#[command(
    ...
    long_about = "查看服务状态、管理账号与配置。\n\n示例：\n  kam status\n  kam account list --tag prod\n  kam config show --effective"
)]
struct Cli {
    /// 管理面 socket 路径，默认读取配置文件。
    #[arg(long, global = true, value_name = "PATH", env = "KAM_ADMIN_SOCKET")]
...
enum Command {
    ...
    /// 查看账号池调度评分。
    #[command(
        after_help = "示例：\n  kam pool --explain\n  kam pool --model claude-sonnet-4 --watch"
    )]
    Pool {
    ...
    /// 上游网络与账号诊断。
    #[command(after_help = "示例：\n  kam diagnose endpoints\n  kam diagnose account --all")]
    Diagnose {
        #[command(subcommand)]
        command: Option<DiagnoseCommand>,
    },
    /// 查询上游可用订阅计划。
    #[command(after_help = "示例：\n  kam subscriptions\n  kam subscriptions acc_7f3a2b1c")]
...
enum ModelMapCommand {
    /// 列出全部映射规则。
    #[command(after_help = "示例：\n  kam model-map list\n  kam --json model-map list")]
    List,
    /// 测试客户端模型名会命中的规则。
    #[command(
        after_help = "示例：\n  kam model-map test claude-sonnet-4\n  kam model-map test gpt-5"
    )]
    Test { model: String },
}
...
enum TaskCommand {
    /// 立即运行一个任务。
    #[command(
        after_help = "示例：\n  kam tasks run status_check\n  kam tasks run model_cache_refresh"
    )]
    Run { name: String },
}
...
enum DiagnoseCommand {
    ...
    #[command(
        after_help = "示例：\n  kam diagnose endpoints\n  kam diagnose endpoints --region us-west-2"
    )]
    Endpoints {
    ...
    /// 拉取模型并发一次真实推理验证账号存活。
    #[command(
        after_help = "示例：\n  kam diagnose account acc_7f3a2b1c\n  kam diagnose account --all --concurrency 4"
    )]
    Account {
        id: Option<String>,
...
enum ConfigCommand {
    ...
    /// 打印全部文件路径。
    #[command(after_help = "示例：\n  kam config path\n  kam --json config path")]
    Path,
    /// 手动触发重载。
    #[command(after_help = "示例：\n  kam config reload\n  kam --json config reload")]
    Reload,
    /// 用 $VISUAL/$EDITOR 打开配置并在保存后校验、重载。
    #[command(after_help = "示例：\n  kam config edit\n  EDITOR=vim kam config edit")]
    Edit,
    /// 只校验配置，不应用。
    #[command(after_help = "示例：\n  kam config validate\n  kam config validate ./config.toml")]
    ...
    match cli.command.unwrap_or(Command::Status { watch: false }) {
        ...
        Command::Version => {
            let value = serde_json::json!({
                ...
                "codewhisperer":kam_kiro::endpoint::CODEWHISPERER_URL,
                "amazonq":kam_kiro::endpoint::AMAZONQ_URL
            });
            if cli.json {
                print_json(&value)?;
            } else {
                ...
                println!("CodeWhisperer {}", kam_kiro::endpoint::CODEWHISPERER_URL);
                println!("AmazonQ        {}", kam_kiro::endpoint::AMAZONQ_URL);
            }
        }
        ...
        Command::Config(ConfigCommand::Reload) => {
            ...
            if cli.json {
                ...
                for field in result.needs_restart {
                    println!("注意：{field} 需重启 kamd 才能生效");
                }
            } else {
                println!(
                    "重载失败，已保留原配置：{}",
