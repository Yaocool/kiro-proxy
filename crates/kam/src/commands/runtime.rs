...
pub enum ApiKeyCommand {
    #[command(after_help = "示例：\n  kam apikey list\n  kam --json apikey list")]
    List,
    #[command(
        after_help = "示例：\n  kam apikey add --name ci\n  kam apikey add --name team --credits-limit 100"
    )]
    Add {
        #[arg(long)]
        name: String,
    ...
    #[command(
        after_help = "示例：\n  kam apikey rm ak_ab12 --yes\n  kam --json apikey rm ak_ab12 --yes"
    )]
    Rm {
        id: String,
        #[arg(long)]
    ...
    #[command(
        after_help = "示例：\n  kam apikey enable ak_ab12\n  kam --json apikey enable ak_ab12"
    )]
    Enable { id: String },
    #[command(
        after_help = "示例：\n  kam apikey disable ak_ab12\n  kam --json apikey disable ak_ab12"
    )]
    Disable { id: String },
    #[command(
        after_help = "示例：\n  kam apikey limit ak_ab12 --credits 100\n  kam apikey limit ak_ab12 --credits 0"
    )]
    Limit {
        id: String,
        #[arg(long)]
    ...
    #[command(
        after_help = "示例：\n  kam apikey usage ak_ab12\n  kam --json apikey usage ak_ab12"
    )]
    Usage { id: String },
    #[command(
        after_help = "示例：\n  kam apikey history ak_ab12\n  kam apikey history ak_ab12 --tail 200"
    )]
    History {
        id: String,
    ...
    #[command(
        after_help = "示例：\n  kam apikey reset-usage ak_ab12 --yes\n  kam --json apikey reset-usage ak_ab12 --yes"
    )]
    ResetUsage {
        id: String,
        #[arg(long)]
...
pub enum WebhookCommand {
    #[command(after_help = "示例：\n  kam webhook list\n  kam --json webhook list")]
    List,
    #[command(after_help = "示例：\n  kam webhook test alerts\n  kam webhook test --all")]
    Test {
        name: Option<String>,
        #[arg(long, conflicts_with = "name")]
        all: bool,
    },
    #[command(after_help = "示例：\n  kam webhook logs\n  kam webhook logs --tail 200")]
    Logs {
        #[arg(long, default_value_t = 50)]
        tail: usize,
    },
...
async fn mutate_config(
    ...
    let config: kam_core::config::Config =
        toml::from_str(&output).context("修改后的配置无法解析")?;
    config.validate().context("修改后的配置校验失败")?;
    kam_store::atomic::write_bytes_atomically(&path, output.as_bytes(), Some(0o600)).await?;
    ...
    kam_store::atomic::write_bytes_atomically(&path, raw.as_bytes(), Some(0o600)).await?;
...
pub async fn validate_config(file: Option<&str>) -> Result<()> {
    ...
    let config = kam_store::config_loader::load_config(&path)
        .await
...
pub async fn edit_config(client: &mut AdminClient) -> Result<()> {
    ...
    if let Err(error) = validate_config(Some(path.to_string_lossy().as_ref())).await {
        kam_store::atomic::write_bytes_atomically(&path, &original, Some(0o600)).await?;
...
pub async fn show_models(client: &mut AdminClient, mapped: bool, json: bool) -> Result<()> {
    ...
    let routes = models
        ...
        .map(|model| {
            let route = kam_translate::model::map_model(
                model,
...
async fn effective_config(client: &mut AdminClient) -> Result<kam_core::config::Config> {
...
pub fn print_topic(topic: &str) -> Result<()> {
    let text = match topic {
        "balance" => {
            "账号池评分 = active_ratio×weight_active + used_credit_ratio×weight_credit + recent_idle_penalty×weight_idle。\n分数越低越优先；随后加入小幅随机抖动，避免并发请求集中到同一账号。\n用 `kam pool --watch --explain` 查看实时评分明细。"
        }
        "model-map" => {
            "模型映射按 priority 从小到大匹配。source_models 支持一个 `*` 通配符；replace/alias 选首个目标，loadbalance 按 weights 随机。\n用 `kam model-map list` 查看规则，`kam model-map test <model>` 验证命中结果。"
        }
        "sso" => {
            r#"单账号：`printf '%s\n' "$PASSWORD" | kam account add-sso --email user@example.com --start-url https://example.awsapps.com/start --password-stdin`。
批量：CSV 仅含 email,password 两列，运行 `kam account add-sso --batch accounts.csv --start-url <url> -c 1`。
若需要验证码或自动化页面变化，增加 `--headful` 手工完成；slim 构建不含 SSO。"#
        }
        "config" => {
            "配置默认位于 $KAM_HOME/config.toml，修改后热重载；server.host/port、admin.socket 和 TLS 监听变更需要重启。\n`kam config validate [file]` 只校验，`kam config edit` 保存后校验并重载，`kam config show --effective` 查看合并默认值的结果。"
        }
        "apikey" => {
            "API key 限额采用在途预留：请求进入时预留估算 credits，结束后按上游实际用量结算，避免并发突破限额。\n统计包含总量、日维度、客户端模型、原模型、Kiro 模型和路径；`kam apikey usage <id>` 与 `history` 可查询。"
        }
        _ => {
            return Err(anyhow!(
