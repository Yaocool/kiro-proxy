...
pub enum AccountCommand {
    /// 列出账号。
    #[command(
        long_about = "列出账号。\n\n示例：\n  kam account list\n  kam account list --tag prod --enabled-only"
    )]
    List {
        /// 只显示带该标签的账号。
    ...
    /// 显示单账号详情。
    #[command(
        long_about = "显示账号详情，不显示 token。\n\n示例：\n  kam account show acc_7f3a\n  kam account show alice@example.com"
    )]
    Show {
        /// 账号 ID 或邮箱。
        id: String,
    ...
    #[command(
        after_help = "示例：\n  kam --json account export > accounts.json\n  kam --json account export --redact"
    )]
    Export {
    ...
    /// 删除账号。
    #[command(
        long_about = "删除账号。\n\n示例：\n  kam account rm acc_7f3a2b1c --yes\n  kam account rm user@example.com --yes"
    )]
    Rm {
        /// 账号 ID 或邮箱。
        id: String,
    ...
    /// 启用账号。
    #[command(
        after_help = "示例：\n  kam account enable acc_7f3a2b1c\n  kam account enable user@example.com"
    )]
    Enable {
        /// 账号 ID 或邮箱。
        id: String,
    ...
    /// 停用账号。
    #[command(
        after_help = "示例：\n  kam account disable acc_7f3a2b1c\n  kam account disable user@example.com"
    )]
    Disable {
        /// 账号 ID 或邮箱。
        id: String,
    ...
    /// 增删标签。
    #[command(
        long_about = "增删标签，可同时操作。\n\n示例：\n  kam account tag acc_7f3a --add prod --add pro\n  kam account tag acc_7f3a --rm dev"
    )]
    Tag {
        /// 账号 ID 或邮箱。
        id: String,
    ...
    /// 重新生成设备标识。
    #[command(
        long_about = "重新生成 machine_id。仅在怀疑当前组合被标记时使用。\n\n示例：\n  kam account regen-machine-id acc_7f3a2b1c\n  kam account regen-machine-id user@example.com"
    )]
    RegenMachineId {
        /// 账号 ID 或邮箱。
        id: String,
    ...
    /// 立即刷新账号 token。
    #[command(
        after_help = "示例：\n  kam account refresh acc_7f3a2b1c\n  kam account refresh --all"
    )]
    Refresh {
        id: Option<String>,
    ...
    /// 清除冷却、封禁与额度耗尽标记。
    #[command(
        after_help = "示例：\n  kam account reset-health acc_7f3a2b1c\n  kam account reset-health --all"
    )]
    ResetHealth {
        id: Option<String>,
        #[arg(long, conflicts_with = "id")]
    ...
    match command {
        AccountCommand::List {
            ...
            if json {
                ...
                println!("暂无账号。用 `kam account import` 添加。");
            } else {
                print!(
                    "{}",
                    render_table(
